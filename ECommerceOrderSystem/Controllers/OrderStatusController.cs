...
public class OrderStatusController(IOrderService orders, ILogger<OrderStatusController> logger) : Controller
{
    ...
    public async Task<IActionResult> Update(Guid id, OrderStatus status)
    {
        if(status is not (OrderStatus.Shipped or OrderStatus.Delivered))
            return RedirectWithError(id, "Administrators can only update orders to Shipped or Delivered.");
    ...
    public async Task<IActionResult> UpdateOwn(Guid id, OrderStatus status)
    {
        if(status is not (OrderStatus.Paid or OrderStatus.Cancelled))
            return RedirectWithError(id, "Customers can only pay for or cancel their own orders.");
    ...
    private async Task<IActionResult> ApplyAsync(Guid id, OrderStatus status, string? customerId = null)
    {
        ...
        if(result.NotFound)
        {
            logger.LogWarning("Order {OrderId} status update to {Status} failed because the order was not found.", id, status); return NotFound();
        }
        if(result.Succeeded)
            logger.LogInformation("Order {OrderId} status was updated to {Status}.", id, status);
        else
            logger.LogWarning("Order {OrderId} status update to {Status} failed: {Message}", id, status, result.Message);
